...
class BuildLines:
    ...
    def handle_more_level_lines(self, node):
        ...
        pt2 = Point(parent.shape.right + SPACING/2, parent.shape.center[1])
        self.update_lines(node, pt1, pt2)

        pt3 = Point(node.shape.left - SPACING/2, node.shape.center[1])
        pt4 = Point(node.shape.left, node.shape.center[1])
