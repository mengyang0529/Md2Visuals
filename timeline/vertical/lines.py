...
class BuildLines:
    ...
    def handle_level_1_lines(self, node):
        ...
        if idx == 0:
            ...
            pt1 = Point(prev_node.shape.center[0], prev_node.shape.bottom)

        pt2 = Point(node.shape.center[0], node.shape.top)
        self.update_lines(node, pt1, pt2)

    def handle_level_2_lines(self, node):
        parent = node.parent
        if node.level == 2:
            pt1 = Point(node.shape.center[0], parent.shape.center[1])
        else:
            if node.id == parent.children[0].id:
                pt1 = Point(node.shape.center[0], parent.shape.bottom)
            else:
                pt1 = Point(node.shape.center[0], parent.shape.bottom + DETAIL_SPACING)
        pt2 = Point(node.shape.center[0], node.shape.top)
        self.update_lines(node, pt1, pt2)
        ...
        if node.id == parent.children[-1].id:
            if node.level == 2:
                if node.is_direction:
                    pt3 = Point(parent.shape.left, parent.shape.center[1])
                else:
                    pt3 = Point(parent.shape.right, parent.shape.center[1])
                pt4 = Point(node.shape.center[0], parent.shape.center[1])
            else:
                pt3 = Point(parent.shape.center[0], parent.shape.bottom + DETAIL_SPACING)
                pt4 = Point(node.shape.center[0], parent.shape.bottom + DETAIL_SPACING)
